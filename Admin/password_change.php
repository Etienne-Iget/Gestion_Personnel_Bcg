...
if (trim($_POST['email_verif'])==='' OR trim($_POST['password_defaut'])==='' OR trim($_POST['password_New'])==='' OR trim($_POST['Confirmation'])==='') {
	...
	if ($password_defaut == 'admin1234') {
	 	...
	 	$requser = $bdd->prepare('SELECT * FROM super_admin WHERE  emailAdmin =?');
		$requser->execute(array($email_verif));

		$user = $requser->fetch();

		if (isset($password_New) AND !empty($password_New) AND isset($Confirmation) AND !empty($Confirmation) And $password_New!=$user['passwordAdmin']) {
			...
			$insertpassword=$bdd->prepare("UPDATE super_admin SET passwordAdmin = ? WHERE emailAdmin = ?");

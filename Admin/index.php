...
$bdd = new PDO('mysql:host=localhost;dbname=administration', 'root', 'Iget1209',array(PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION));
	...
	if (isset($_POST['Connexion'])) {
		...
		if (!empty($emailAdmin) AND !empty($passwordAdmin)) {

			$verifAdmin = $bdd->prepare('SELECT * FROM super_admin WHERE emailAdmin = ? AND passwordAdmin = ?');
			$verifAdmin->execute(array($emailAdmin,$passwordAdmin));
			$userinfo = $verifAdmin -> fetch();
				
			if($verifAdmin -> rowcount() == 1){

				$req= $bdd -> prepare(" INSERT INTO log_super_admin (emailAdmin, passwordAdmin) VALUES(:emailAdmin, :passwordAdmin)");
				$req->execute(array('emailAdmin'=>$emailAdmin,'passwordAdmin'=>$passwordAdmin));

				$_SESSION['idAdmin']=$userinfo['idAdmin'];
				$_SESSION['nomAdmin']=$userinfo['nomAdmin'];
				$_SESSION['postnomAdmin']=$userinfo['postnomAdmin'];
				$_SESSION['emailAdmin']=$userinfo['emailAdmin'];
...
<html>
	...
	<title>Connexion &mdash; DIRECTION </title>
			...
			<div class="row">
				...
				<div class="col-xs-8 text-right menu-1">
					<ul>
						...
						<li class="has-dropdown"><a href="#">Connectez-vous!</a>

							<ul class="dropdown">
								
								<li><a style="color: black;" class="btn btn-warning" href="../connexion.php">Admin</a></li>
								
								<li><a style="color: black;" class="btn btn-primary" href="http://localhost/Employe/index.php">Employer</a></li>
							</ul>
								...
								<div class="tab">
									...
									<div class="tab-content">
										...
										<p style="color: red;"><em><?php if(isset ($_POST['Connexion']) AND isset($return)) echo $return; ?></em></p>
											...
											<p>Compte du Super Administrateur <a href="superAdmin.php" style="color: red;"></a></p><br>

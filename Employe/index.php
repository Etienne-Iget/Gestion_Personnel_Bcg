...
$bdd = new PDO('mysql:host=localhost;dbname=bcg', 'root', 'Iget1209',array(PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION));
	...
	if (isset($_POST['Connexion'])) {
		...
		if (!empty($email) AND !empty($password)) {
			...
			$verifAdmin = $bdd->prepare('SELECT * FROM employe WHERE email = ? AND password =?');
			...
			if($verifAdmin -> rowcount() == 1){
				...
				if($_SESSION['etatService']=='En Service' OR $_SESSION['etatService'] =='En Conge' OR $_SESSION['etatService'] =='En Vacances'){
				...
				// enregistrement log
					...
					$req= $bd -> prepare(" INSERT INTO log_employe (matricule, nom, email, heureConnexion) VALUES(:matricule, :nom, :email, :heureConnexion)");
					$req->execute(array('matricule'=>$matricule, 'nom'=>$nom, 'email'=>$email , 'heureConnexion'=>$heureConnexion));


						header("location:information.php");

						}else {
							$return = "Vous etes suspendu ou Hors service";
						}
...
<html>
	...
	<title>Connexion &mdash; Employé </title>
